...
// Test case for getTask() method: Case-1
  it('should get todos from api end and set todos (getTask method)', ()=>{
    ...
    spyOn(taskService, 'getTodoByPageIn').and.returnValue(of(mockResponse));
    component.getTask();

    expect(taskService.getTodoByPageIn).toHaveBeenCalledOnceWith(currentPage, pageSize);
    expect(component.incompleteTasks()).toBe(mockResponse.todos);
  ...
  it('should alert and set errorMessage if fetch failed (getTask method)', () => {
    ...
    spyOn(taskService, 'getTodoByPageIn').and.returnValue(throwError(()=> mockError));
    component.getTask();
  ...
  it('should update task with completed if task with id is found (checked method)', () =>{
    ...
    component.incompleteTasks.set(mockTask);
  ...
  it('should have been called ngOnInit', () =>{
    ...
    expect(component.getTask).toHaveBeenCalled();
    expect(component.incompleteTasks()).toEqual(mockTask);
  });

// Test case for goToNext() method
  it('should go to next page if currentPage < totalPages (goToNext method)', ()=>{
...
// Test case for goToPrev() method
  it('should go to previous page if currentPage > 1 (goToPrev method)', ()=>{
